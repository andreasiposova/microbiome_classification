...
def grid_search_rf(X_train, X_test, y_train, y_test, X_val, y_val, file_name):
    ...
    param_grid = {
        'n_estimators': [10, 20], #np.linspace(20, 1000, 50, dtype=int),
        'max_depth': [5], #np.linspace(2, 40, 3, dtype=int),
        'min_samples_split':[2, 4], #np.linspace(2, 40, 3, dtype=int),
        'min_samples_leaf': [4], #np.linspace(2, 40, 3, dtype=int),
        'max_features': ['auto'],#['auto', 'sqrt', 'log2'], # 'sqrt', 'log2']
        'random_state': [1234]
    }
    ...
    for i in range(len(grid_search.cv_results_['params'])):
        ...
        #for each file, get the metrics on the test set and for each combination of params and choose the one with the highest roc_auc
        # and then apply on the huadong val set
    ...
    # Return the results
    return test_scores, val_scores, best_train_est_test_eval, best_estimator_params_on_train, y_test, y_pred
...
def run_rf_tuning(data_name, filepath, h1_filepath, h2_filepath):
    ...
    for key in data:
        ...
        y_val = y_h1 + y_h2
        common_cols = set(X_test.columns).intersection(X_val.columns)
        X_val = X_val[common_cols]
        scores, val_scores, best_results_on_train, best_estimator, y_test, y_pred = grid_search_rf(X_train, X_test, y_train, y_test, X_val, y_val, file_name=key)
        full_results.append([str(key),best_estimator, best_results_on_train])
        visualize_results(scores, data_name, key, y_test, y_pred)
        #print(full_results) #best estimator found on the train set through grid seach and 10fold CV
    ...
    for i in full_results:
        ...
        results_table.drop(['oob_score', 'min_weight_fraction_leaf', 'bootstrap',
                            'ccp_alpha', 'class_weight', 'min_impurity_decrease',
                            'min_impurity_split', 'max_leaf_nodes', 'max_samples',
                            'verbose', 'warm_start'], inplace=True, axis=1)
    ...
    with open(os.path.join(Config.LOG_DIR, data_name, f"best_estimator_on_train_eval_on_test_results.txt"), "w") as f:
        f.write(latex_table)
