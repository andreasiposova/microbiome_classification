...
def perform_classification(X_train, X_test, X_val, y_train, y_test, y_val, params, file_name, group, fal=True):
    full_results_train = []
    full_results_test = []
    full_results_val = []
    ...
    results_df = results_df.append(train_set_res, ignore_index=True)
    results_df = results_df.append(test_set_res, ignore_index=True)
    results_df = results_df.append(val_set_res, ignore_index=True)
...
def get_all_results(data_name, filepath, group, select_features=True):
    ...
    if select_features == True:
        ...
        results = perform_classification(X_train, X_test, X_val, y_train, y_test, y_val, params, file_name="selected_features", group=group, fal=False)
        print(results)
    if select_features == False:
        params = get_best_params(FUDAN, group, 'all_features', param_file='best_params')
        results = perform_classification(X_train, X_test, X_val, y_train, y_test, y_val, params, file_name="all_features", group=group, fal=False)
        print(results)

get_all_results(FUDAN, fudan_filepath, 'old', True)