...
def get_rf_scores_params(test_scores):
    ...
    n_estimators_mean_metrics = params_df[['n_estimators', 'accuracy', 'precision', 'recall', 'roc_auc', 'f1', 'f2']].groupby('n_estimators').mean()
    n_estimators_mean_metrics.reset_index(inplace=True)
    max_depth_mean_metrics = params_df[['max_depth', 'accuracy', 'precision', 'recall', 'roc_auc', 'f1', 'f2']].groupby('max_depth').mean()
    max_depth_mean_metrics.reset_index(inplace=True)
    max_features_mean_metrics = params_df[['max_features', 'accuracy', 'precision', 'recall', 'roc_auc', 'f1', 'f2']].groupby('max_features').mean()
    max_features_mean_metrics.reset_index(inplace=True)
    min_samples_leaf_mean_metrics = params_df[['min_samples_leaf', 'accuracy', 'precision', 'roc_auc', 'recall', 'f1', 'f2']].groupby('min_samples_leaf').mean()
    min_samples_leaf_mean_metrics.reset_index(inplace=True)
    min_samples_split_mean_metrics = params_df[['min_samples_split', 'accuracy', 'precision', 'roc_auc', 'recall', 'f1', 'f2']].groupby('min_samples_split').mean()
...
def sensitivity_plot(hp_mean_metrics, data_name, file_name):
    ...
    plt.title(file_name)
    plt.tight_layout()
    plt.savefig(os.path.join(Config.PLOTS_DIR, data_name, file_name, f"{hyperparam_name}_rf_sensitivity_gridsearch_avg.png"))
    plt.show()
