...
def grid_search_rf(X_train, X_test, y_train, y_test, X_val, y_val, data_name, file_name, group):
    ...
    grid_search = GridSearchCV(estimator=knn, param_grid=param_grid, cv=cv, scoring=scoring, refit='roc_auc',
    ...
    grid_search_train_test_plot(train_scores_gridsearch, test_scores_gridsearch, data_name, group, file_name, "KNN")
...
def run_rf_tuning(data_name, filepath, group, select_features = True):
    ...
    if not os.path.exists(str(Config.LOG_DIR) + "/" + str(data_name) + "/" + group + "/" + str(file_name)):
        os.makedirs(os.path.join(Config.LOG_DIR, data_name, group, file_name))
    with open(os.path.join(Config.LOG_DIR, data_name, group, file_name, f"KNN_best_params.txt"), "w") as f:
...
#   results_val_table.drop(untuned_params, inplace=True, axis=1)
    save_result_table(results_train_table, results_test_table, results_val_table, data_name, group, file_name, table_name="KNN_best_results")
